...
class AttackEngine:
    ...
    def __init__(self, head_slot: int, alpha: float):
        ...
        self.silent_fork: bool = True # True if no forking is happening, otherwise denotes whether the honest validators vote for the
    ...
    def propose(self, slot: int) -> list[BeaconChainAction]:
        ...
        if self.head_slot != self.private_head_slot: # Finishing forking
            assert self.adv_votes + BOOST > self.honest_votes, f"Not sufficient votes to reorg {self.adv_votes=} {self.honest_votes=}"
            ...
            for last_slot, block_slot in zip(slots_with[:-1], slots_with[1:]):
                actions.append(Propose(slot=slot, block_slot=block_slot, parent_block_slot=last_slot))
            self.withheld_blocks = []
        else:
            actions.append(Propose(slot=slot, block_slot=slot, parent_block_slot=self.head_slot))
